...
class ConvLSTM(nn.Module):
    ...
    def forward(self, level1):
        ...
        out2_op = (self.conv_squeeze(out2_op)).view(lb,lc,lw,lh)

        out3h,out3v = self.cfm(level3, out2_op)
        out3_op = torch.cat((out3h,out3v),1).view(lb,lc*2,lw,lh)
        out3_op = (self.conv_squeeze(out3_op)).view(lb,lc,lw,lh)

        out4h,out4v = self.cfm(level4, out3_op)
        out4_op = torch.cat((out4h,out4v),1).view(lb,lc*2,lw,lh)
        out4_op = (self.conv_squeeze(out4_op)).view(lb,lc,lw,lh)

        out5h,out5v = self.cfm(level5, out4_op)
        out5_op = torch.cat((out5h,out5v),1).view(lb,lc*2,lw,lh)
        out5_op = (self.conv_squeeze(out5_op)).view(lb,lc,lw,lh)

        # out1 = (self.conv1x1(out2_op.transpose(0,1))).transpose(0,1)
        out2 = (self.conv1_1(out2_op.transpose(0,1))).transpose(0,1)
        out3 = (self.conv1_1(out3_op.transpose(0,1))).transpose(0,1)
        out4 = (self.conv1_1(out4_op.transpose(0,1))).transpose(0,1)
        out5 = (self.conv1_1(out5_op.transpose(0,1))).transpose(0,1)

        new_c = (self.conv1_1(out5h.transpose(0,1))).transpose(0,1)
