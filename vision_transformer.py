...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        #-----deyiwang
        gst_reduction = 4
        self.GSTAttention = nn.Module()
        self.GSTAttention.fc1 = nn.Linear(num_features, num_features // gst_reduction, bias=False)
        self.GSTAttention.fc2 = nn.Linear(num_features // gst_reduction, num_features, bias=False)
        self.GSTAttention.sigmoid = nn.Sigmoid()

    def forward_features(self, x, flag = 2):
        ...
        if flag == 2:
            ...
            # print(x.shape)
            new_new = pos_embed.permute(0, 2, 1)
            batch_size, channel_num, feat_len = new_new.size()
            y = torch.mean(new_new, dim=2)
            y = self.GSTAttention.fc1(y)
            y = F.relu(y)
            y = self.GSTAttention.fc2(y)
            y = self.GSTAttention.sigmoid(y).view(batch_size, channel_num, 1)
            new_new = new_new * y.expand_as(new_new)
            pos_embed = new_new.permute(0, 2, 1)
...
def vit_b_16(input_shape=[224, 224], pretrained=False, num_classes=1000):
    ...
    if pretrained:
        ...
        pretrained_dict = torch.load("model_data/vit-patch_16.pth")
...
def test_vit_b_16():
    # try:
    # 测试不使用预训练权重
    model1 = vit_b_16(pretrained=False)
    print("Test 1 Passed: Model initialized without pretrained weights.")

    # 测试使用预训练权重
    # （确保 "model_data/vit-patch_16.pth" 存在且格式正确）
    model2 = vit_b_16(pretrained=True)
    print("Test 2 Passed: Model initialized with pretrained weights.")
    
    # 测试不同的 num_classes
    num_classes = 10
    model3 = vit_b_16(pretrained=False, num_classes=num_classes)
    input_tensor = torch.randn(1, 3, 256, 256)  # 创建一个随机输入
    output = model3(input_tensor)
    print(output.shape)
    assert output.shape[-1] == num_classes, "Output shape mismatch."
    print("Test 3 Passed: Model output shape is correct for different num_classes.")
